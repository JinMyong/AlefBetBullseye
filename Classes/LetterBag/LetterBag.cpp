...
int LetterBag::getRandomLetterFormBagLimitRepeats()
{
    ...
    CCLog("Random no repeat letter:%d %s", letterIndex, HebrewUtils::getInstance()->getLetterName(letterIndex).c_str());
        ...
        while ((randLetter == excludeLetter) || soundAlike || betExclusion) {
            ...
            do {
                ...
                if (randLetter != excludeLetter) {
                    soundAlike = HebrewUtils::getInstance()->isSoundaAlikesVowel(randLetter, excludeLetter);
                }
                
                // Exclude vowels when letter is a bet
                if (excludeLetter == BET_IDX) {
                    betExclusion = HebrewUtils::getInstance()->isVowel(randLetter);
                }
            } while (alreadyIn);
        }
...
void LetterBag::logAllLettersInBag()
{
    for (int i = 0; i < bagOfLetter->count(); i++) {
        ...
        CCLog("%d %s", letterIndex->getValue(), HebrewUtils::getInstance()->getLetterName(letterIndex->getValue()).c_str());
    }
}
