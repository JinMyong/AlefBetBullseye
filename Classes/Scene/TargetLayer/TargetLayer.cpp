    ...
    for (int rowIndex = 0; rowIndex < rows; rowIndex++) {
        ...
        for (int colIndex = 0; colIndex < cols ; colIndex++) {
            ...
            if (targetLetters->count() > 0) {
                ...
                string letterString = HebrewUtils::getInstance()->getKeyMapFont(letterIndex);
                letterSprite = CCLabelTTF::create(letterString.c_str(), fontName.c_str(), targetSprite->getContentSize().width);
