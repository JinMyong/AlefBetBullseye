...
void ChooseLetterScene::createLetterBoard()
{
    ...
    HebrewUtils* hebrewUtils = HebrewUtils::getInstance();
    ...
    if (fontType) {
        fontName = HEBREW_NOR_FONT;
        factor = 1.2;
    }else
    {
        fontName = HEBREW_CUR_FONT;
        factor = 1.2;
    }
    ...
    for (int letterIndex = 0; letterIndex < 43; letterIndex++) {
        if (xIndex % NUM_COLUMNS == 0) {
            ...
            if (letterIndex > LAST_CONSONANT_IDX) {
                vowelRowCount++;
//                fontSize = 76 * SCALE;  // LETTER_FONT_SIZE_VOWEL;
            }
        }
        
        string letterString = hebrewUtils->getKeyMapFont(letterIndex);
        ...
        letterSprite->setDimensions(CCSize(fontSize, fontSize));
...
void ChooseLetterScene::letterSpriteSelected(cocos2d::CCLabelTTF *letterSprite)
{
    ...
    string letterName = hebrewutils->getLetterName(tag);
...
//    const int yOffset = 450 * SCALEY;
    ...
    // Get the "name" of the letter
    string letterName = HebrewUtils::getInstance()->getLetterName(lastLetterIndex);
